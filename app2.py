...
st.subheader("Correlation-Based vs Firefly Algorithm on Logistic Regression using Diabetes Dataset")

st.write("""
This application compares two feature selection methods applied to the Diabetes dataset:
...
@st.cache_data
def load_data():
    # Try to use the diabetes dataset from sklearn if available, otherwise use a sample dataset
    try:
        ...
        df = pd.DataFrame(np.random.randn(n_samples, n_features), columns=feature_names)
        
        # Create synthetic target with some correlation to features
        target = 0.3 * df['bmi'] + 0.2 * df['bp'] - 0.1 * df['s1'] + 0.4 * df['s2'] + np.random.randn(n_samples) * 0.5
